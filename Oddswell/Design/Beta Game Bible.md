...
- Phase 1H.1 freezes the first Match Winner rules, Phase 1H.2 implements the offer contract, Phase 1H.3 persists one machine-local accepted request plus stake debit, Phase 1H.4 persists one separate idempotent game-start lock decision, Phase 1H.5 links that chain to the exact sealed Season 1, Game 1 result, Phase 1H.6 derives the Harbor-selected/Mesa-winner decision as `lost`, `GrossReturnDue = 0`, status `decided_pending_apply`, Phase 1H.7 creates one separate immutable `settled_lost` finalization with gross return applied `0`, and Phase 1H.8 exposes that exact chain through one independently validated read-only Operations reconciliation/history card; see [[Development/Reports/Phase 1H8 Read Only Exact Loss Reconciliation History]]. The prior decision remains immutable as `decided_pending_apply`; the two-entry ledger, balance `60`, cooldown, generic Odds Bucks projection, replay, and basketball outputs remain unchanged. No ledger entry is added and the card has no controls. No player-facing wager route/history, winning-return application, payout, refund, void, correction, prop market, account, trusted clock, backend, deployment, or brain change is implemented.
