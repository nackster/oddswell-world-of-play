...
- Phase 1H.26A through 1H.26G establish one active canonical local-beta opener from immutable schedule, equal-public commitment, exact Match Winner offer, read-only booth presentation, one server-authoritative `40` Odds Bucks request/debit, its cold-restored pending receipt, and one separate immutable server-owned game-start lock; see [[Development/Reports/Phase 1H26G Server Owned Exact Canonical Game Start Lock]]. The H26G transition accepts no caller input, reloads exact current evidence, runs only at exact H26A tipoff, preserves the request, ledger `2/-40/60`, balance, cooldown, upstream records, and first-written times, and is duplicate-safe on cold retry. Invalid or failed-write evidence mutates nothing. After lock, the booth shows only unavailable/locked with no pending receipt, teams, prices, result, or controls. Normal selection/confirmation, simulation, result, settlement, correction, broader history, later markets, simulator behavior, and brain behavior remain unproved.
