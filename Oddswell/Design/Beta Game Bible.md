...
- Phase 1H.26A through 1H.26F establish one active canonical local-beta opener from immutable schedule, equal-public commitment, exact Match Winner offer, read-only booth presentation, one server-authoritative `40` Odds Bucks request/debit, and its cold-restored read-only pending receipt; see [[Development/Reports/Phase 1H26F Cold Restored Read Only Canonical Pending Wager Receipt]]. The receipt independently requires exact schema-`12` request, ledger, offer, timing, and no-downstream evidence before showing Harbor, stake `40`, potential gross `69`, balance `60`, and `accepted_pending_lock`. Missing-request state preserves the read-only offer; malformed, multiple, migrated, downstream, or late evidence exposes no pending claim and mutates nothing. Normal selection/confirmation, canonical game-start lock, result, settlement, correction, broader history, later markets, simulator behavior, and brain behavior remain unproved.
