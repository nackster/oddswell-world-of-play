...
- Phase 1H.26A through 1H.26H establish one active canonical local-beta opener from immutable schedule, equal-public commitment, exact Match Winner offer, read-only booth presentation, one server-authoritative `40` Odds Bucks request/debit, its cold-restored pending receipt, one separate immutable server-owned game-start lock, and one private immutable execution commitment; see [[Development/Reports/Phase 1H26H Immutable Canonical Active Game Execution Commitment]]. H26H accepts no caller input, reloads exact current evidence, derives its versioned seed only from immutable H26A schedule fields plus the H26B hash, and freezes every current `simulate_scheduled_game` input/version before execution. Request, selection, stake, price, balance, ledger, lock command, and accepted time cannot affect the seed. The seed and full input remain absent from public/player/Admin surfaces. Exact cold retry is duplicate-safe; invalid or failed-write evidence mutates nothing. Normal selection/confirmation, simulation result persistence, wager decision, settlement, correction, broader history, later markets, simulator behavior changes, and new brain behavior remain unproved.
