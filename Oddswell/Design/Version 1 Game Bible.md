...
- **BETA EVIDENCE:** one separate immutable commitment now binds that opener to fresh public `0-0` standings, equal `7`-day rest, existing public fictional roster ratings, published availability, projected minutes, and the frozen `phase0d4-v1` `public_elo_rotation` prediction. It reads no completed archive input and creates no offer; see [[Development/Reports/Phase 1H26B Immutable Equal Public Pregame Prediction Commitment]] and [[Design/Decisions/DEC-017 Match Winner Odds and Stake Defaults]].
- **BETA EVIDENCE:** one separate pre-tipoff canonical Match Winner record binds that schedule and commitment to the approved zero-edge, `10`–`100` by `10`, exact floor-payout offer contract. The stadium ticket booth now supports one deliberately narrow normal action: Harbor City Waves at exactly `40`, armed by a mutation-free first confirmation and submitted by a second parameterless server call. The server derives all identity, odds, time, and command evidence before atomically creating the existing request/debit and exposing the pending receipt. That exact pending request, including a valid pre-tipoff cold restore, schedules one server timer that invokes the existing parameterless H26G lock only at exact H26A tipoff; late starts never catch up or backdate, and H26H-H26P remain manual. Mesa, other stakes, generalized submission, multiple requests, and later markets remain unproved; see [[Development/Reports/Phase 1H26C Immutable Canonical Match Winner Offer Record]], [[Development/Reports/Phase 1H26Q Interactive Canonical Match Winner Selection and Pre Commit Bet Slip Review]], [[Development/Reports/Phase 1H26R Exact Harbor 40 Canonical Wager Placement]], and [[Development/Reports/Phase 1H26S Automatic Exact Canonical Tipoff Lock]].
