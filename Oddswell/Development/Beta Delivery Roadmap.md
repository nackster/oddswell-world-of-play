...
- Next candidate gate: **IMMUTABLE SEALED-RESULT LINKAGE WITHOUT PAYOUT**. The Scope Director must confirm the exact subphase before implementation. Player props remain later gates. Sundale remains a working name.
...
| 8 | 1H — Wager locking and settlement | Place approved wagers and receive exact outcomes | ACTIVE — LOCAL REQUEST + LOCK COMPLETE / NO RESULT OR SETTLEMENT |
...
- **NEXT CANDIDATE:** bind one exact sealed authoritative result to the locked wager without paying, refunding, or otherwise mutating the ledger. Scope Director confirmation is required.
