...
- Phase 1K.6 Sundale handoff correction and fresh component-one proof: **COMPLETE - COMPONENT 1 PASSED; COMPONENTS 2-8 NOT REACHED**; see [[Development/Reports/Phase 1K6 Sundale Handoff Correction and Fresh Component One Proof]].
- Next gate: **PHASE 1K.7 - MEASURE SHARED-SESSION MEMBERSHIP, STARTER-APPEARANCE AGREEMENT, AND THEN THE FIRST NORMAL MOVEMENT**. Stop at the first component-2 failure and never begin component 3 in that phase.
...
9. **NEXT - Phase 1K.7 COMPONENT-2 MEASUREMENT:** first prove or fail at normal owner membership in the passive peer's listen session and owner/peer starter-appearance agreement. Only if both pass may the owner move once by normal controls; stop before component 3.
10. **PENDING - later Phase 1K gates:** select only from the next measured fail-fast boundary; do not invent a generalized tutorial, orchestration framework, or new authority.
10. **CONDITIONAL — reopen Phase 1J:** only if measured first-hour or player evidence shows that required athlete distinctions cannot be represented by existing public fields and clearer presentation.
...
Phase 1K.7 should measure the current normal post-chooser path against the already-running passive peer. It must first prove or fail at shared listen-session membership and owner/peer starter-appearance agreement. Only if those pass may it exercise the first normal owner movement required by component 2, then stop before component 3.
