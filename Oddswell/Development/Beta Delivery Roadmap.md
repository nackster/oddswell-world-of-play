...
- Phase 1K.5 fresh non-default preset selection and handoff measurement: **COMPLETE — PERSISTENCE PASSED; NORMAL WORLD HANDOFF FAILED**; see [[Development/Reports/Phase 1K5 Fresh Non Default Preset Selection and Handoff Measurement]].
- Next gate: **PHASE 1K.6 — CORRECT ONLY THE NORMAL CONFIRMATION TARGET TO `SundaleGraybox`, REBUILD, AND REPEAT FRESH COMPONENT-1 PROOF**. Do not begin route movement or component 2 in that phase.
...
8. **NEXT — Phase 1K.6 HANDOFF-TARGET CORRECTION:** change only the chooser confirmation target to the existing packaged `SundaleGraybox` plus `OddsWellLocomotionGameMode`, rebuild, and repeat the fresh component-1 proof. Stop after successful Sundale entry; do not move the player or begin component 2.
9. **PENDING — later Phase 1K gates:** select only from the next measured fail-fast boundary; do not invent a generalized tutorial, orchestration framework, or new authority.
...
Phase 1K.6 should correct only the normal chooser-confirmation target from absent `BlockCourtBenchmark` to the existing packaged `SundaleGraybox` with `OddsWellLocomotionGameMode`, rebuild, and repeat the fresh accepted-network component-1 proof. Stop after successful Sundale entry; do not move the player or begin component 2 in the same phase.
