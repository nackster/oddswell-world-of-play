...
- Next gate: **SCOPE DIRECTOR REVIEW** of the smallest truthful read-only canonical pending receipt. The normal H26D booth remains read-only; input controls, game-start lock, result, settlement, correction, broader history, multiple requests, and every later market remain unproved and unauthorized. Sundale remains a working name.
...
| 8 | 1H — Wager locking and settlement | Reach the physical booth, inspect approved odds, place approved wagers, and receive exact outcomes | ACTIVE — CANONICAL READ-ONLY MATCH WINNER PRESENTATION + LOSS/WIN/PRIOR VOID HISTORY + EXACT NONCANONICAL QA REQUEST-TO-REFUND HISTORY COMPLETE / CANONICAL SELECTION, SUBMISSION, RESULT, SETTLEMENT, CORRECTION, BROADER HISTORY, AND LATER MARKET PROOFS UNPROVEN |
