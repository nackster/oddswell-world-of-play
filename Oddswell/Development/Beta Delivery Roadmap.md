...
- Next gate: **SCOPE DIRECTOR REVIEW** of the smallest authoritative execution step that consumes this exact private commitment. Normal selection/confirmation, simulation result persistence, wager decision, settlement, correction, broader history, multiple requests, and every later market remain unproved and unauthorized. Sundale remains a working name.
...
| 8 | 1H — Wager locking and settlement | Reach the physical booth, inspect approved odds, place approved wagers, and receive exact outcomes | ACTIVE — CANONICAL OFFER, ONE EXACT REQUEST/DEBIT, COLD PENDING RECEIPT, AND EXACT GAME-START LOCK + LOSS/WIN/PRIOR VOID HISTORY + EXACT NONCANONICAL QA REQUEST-TO-REFUND HISTORY COMPLETE / NORMAL SELECTION, RESULT, SETTLEMENT, CORRECTION, BROADER HISTORY, AND LATER MARKET PROOFS UNPROVEN |
