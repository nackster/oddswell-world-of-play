...
- Phase 1K.7 normal shared-session membership measurement: **COMPLETE - COMPONENT 2 FAILED AT MEMBERSHIP; COMPONENTS 3-8 NOT REACHED**; see [[Development/Reports/Phase 1K7 Normal Shared Session Membership Fail Fast Measurement]].
- Next gate: **PHASE 1K.8 - CORRECT ONLY THE NORMAL HANDOFF TO JOIN THE ACCEPTED SUNDALE LISTEN ENDPOINT**. Reprove component 1, stop first at membership, and never begin component 3 in that phase.
...
5. **COMPLETE — Phase 1K.3 DEFAULT CHOOSER FAILED:** accepted peer networking passed, then fresh normal package startup loaded Bootstrap as plain `GameModeBase` with a black viewport and no chooser/cards/preset/appearance state, prompt, modal, or owner SaveGame during `60.091` seconds of zero input. Setup and components 1-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K3 Fresh Accepted Network Setup and Default Character Chooser Fail Fast Measurement]].
6. **COMPLETE — Phase 1K.4 DEFAULT CHOOSER CORRECTION:** one `GlobalDefaultGameMode` entry connects normal Bootstrap startup to the existing approved chooser. Fresh accepted-network proof showed all eight cards for `65.610` seconds with zero input, no auto-confirmation, no appearance SaveGame, and no route handoff. Components 1-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K4 Normal Bootstrap Character Chooser Correction and Fresh Proof]].
7. **COMPLETE — Phase 1K.5 SELECTION/HANDOFF MEASUREMENT:** one Right-arrow input selected `masculine_tone_2`; one Enter confirmation created exactly one valid production appearance save. The normal target `/Game/Maps/BlockCourtBenchmark` was absent, travel failed, and Bootstrap reopened. Components 1-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K5 Fresh Non Default Preset Selection and Handoff Measurement]].
8. **COMPLETE - Phase 1K.6 HANDOFF-TARGET CORRECTION:** the one production target now opens existing packaged `SundaleGraybox`. Fresh automated proof selected and saved `masculine_tone_2` once, entered normal play under `OddsWellLocomotionGameMode`, and stopped with zero movement. Component 1 passed; components 2-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K6 Sundale Handoff Correction and Fresh Component One Proof]].
9. **COMPLETE - Phase 1K.7 COMPONENT-2 MEASUREMENT:** the fresh normal owner repeated component 1 but opened a separate standalone Sundale world. The accepted peer stayed at `clients=1`; mutual visibility, appearance agreement, and movement were not reached. See [[Development/Reports/Phase 1K7 Normal Shared Session Membership Fail Fast Measurement]].
10. **NEXT - Phase 1K.8 SHARED-HANDOFF CORRECTION:** change only the normal post-confirmation handoff so the owner joins the accepted machine-local Sundale listen endpoint. Reprove component 1 and stop first at membership; only after membership passes may the phase measure appearance agreement and one movement.
11. **PENDING - later Phase 1K gates:** select only from the next measured fail-fast boundary; do not invent a generalized tutorial, orchestration framework, or new authority.
12. **CONDITIONAL — reopen Phase 1J:** only if measured first-hour or player evidence shows that required athlete distinctions cannot be represented by existing public fields and clearer presentation.
...
### Phase 1K.3 measurement

**COMPLETE — ACCEPTED PEER NETWORK PASSED; DEFAULT OWNER CHOOSER FAILED DURING SETUP.** Attempt `phase1k3-20260805T023402Z` reused the unchanged package and fresh isolated peer/owner profiles. The peer's exact game PID owned gameplay UDP `127.0.0.1:17777`; trace-control TCP `0.0.0.0:1985` remained the accepted Development debt. The owner launched through the package default with no map URL, `?game=`, gameplay QA flag, input driver, save, or prestate. Bootstrap loaded as plain `GameModeBase`; the viewport was black and no chooser, cards, preset/appearance state, prompt, modal, project UI marker, or owner SaveGame appeared during `60.091` seconds of zero input. Both processes exited normally, isolated profiles were hashed then removed, and ports/processes returned to zero. Setup and components 1-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K3 Fresh Accepted Network Setup and Default Character Chooser Fail Fast Measurement]].

### Phase 1K.4 correction and proof

**COMPLETE — NORMAL DEFAULT STARTUP PRESENTS THE APPROVED CHOOSER.** One `GlobalDefaultGameMode=/Script/OddsWell.OddsWellCharacterSelectionGameMode` entry fixes the measured Bootstrap configuration gap without adding a UI or startup framework. Attempt `phase1k4-20260805T142500Z` rebuilt the Windows Development package, passed the accepted peer `GameNetDriver` boundary, and launched a fresh owner through the normal default path. Bootstrap loaded `OddsWellCharacterSelectionGameMode`; the visible eight-card chooser remained present for `65.610` seconds with zero input, zero appearance-save markers, no owner SaveGame, no auto-confirmation, and no world handoff. Both clients exited normally and active launch paths, ports, and processes returned to zero. Components 1-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K4 Normal Bootstrap Character Chooser Correction and Fresh Proof]].

### Phase 1K.5 measurement

**COMPLETE — PERSISTENCE PASSED; NORMAL WORLD HANDOFF FAILED.** Attempt `phase1k5-20260805T173036Z` reused the unchanged Phase 1K.4 package and fresh isolated profiles. The accepted passive peer reached Sundale on gameplay UDP `127.0.0.1:17777` and received zero input. The owner used one Right-arrow input to select `masculine_tone_2` and one Enter input to confirm. Exactly one production appearance-save marker and one hashed SaveGame prove persistence. Confirmation then targeted absent `/Game/Maps/BlockCourtBenchmark`; Unreal emitted `ClientTravelFailure` and returned to Bootstrap. No route input followed. Components 1-8 remain **NOT REACHED**. See [[Development/Reports/Phase 1K5 Fresh Non Default Preset Selection and Handoff Measurement]].

### Phase 1K.6 correction and proof

**COMPLETE - COMPONENT 1 PASSED; COMPONENTS 2-8 NOT REACHED.** The one production chooser-confirmation target now opens existing packaged `/Game/Maps/SundaleGraybox` while retaining `OddsWellLocomotionGameMode`. Attempt `phase1k6-20260805T235056Z` rebuilt the Windows Development package, used fresh isolated profiles, selected non-default `masculine_tone_2` with one Right input, confirmed once with Enter, created exactly one valid production appearance save, and entered normal Sundale play without travel failure or fallback. The attempt stopped with owner movement inputs `[]` and peer inputs `[]`; it did not prove a shared session, appearance agreement, component 2, or player usability. See [[Development/Reports/Phase 1K6 Sundale Handoff Correction and Fresh Component One Proof]].

### Phase 1K.7 measurement

**COMPLETE - COMPONENT 2 FAILED AT SHARED-SESSION MEMBERSHIP; COMPONENTS 3-8 NOT REACHED.** Attempt `phase1k7-20260806T022019Z` reused the unchanged Phase 1K.6 package and fresh isolated profiles. The normal owner repeated component 1, but the post-confirmation browse opened a separate standalone `SundaleGraybox` world instead of joining the accepted peer endpoint at `127.0.0.1:17777`. The peer stayed at `clients=1`; neither client logged mutual visibility or cross-client appearance agreement. Owner movement inputs and peer inputs were both `[]`. See [[Development/Reports/Phase 1K7 Normal Shared Session Membership Fail Fast Measurement]].

### Next measured gate

Phase 1K.8 should correct only the normal post-confirmation handoff so the owner joins the accepted machine-local Sundale listen endpoint instead of opening a standalone local map. Rebuild and repeat the fresh component-1 path. Stop first at membership; only if it passes may the phase measure mutual visibility, starter-appearance agreement, and one normal movement. Do not begin component 3.
