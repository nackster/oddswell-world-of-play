...
6. **NEXT — Phase 1K.4:** correct only the measured normal-default Bootstrap/chooser failure. The existing approved chooser must appear without auto-selecting a preset or bypassing normal input; prove it in a new fresh accepted-network setup before route work.
7. **PENDING — later Phase 1K gates:** select only from the next measured fail-fast boundary; do not invent a generalized tutorial, orchestration framework, or new authority.
8. **CONDITIONAL — reopen Phase 1J:** only if measured first-hour or player evidence shows that required athlete distinctions cannot be represented by existing public fields and clearer presentation.
...
Phase 1K.4 should make only the smallest normal-default Bootstrap correction required to present the already approved default character chooser. It must not auto-select a preset, bypass normal input, begin the route, or broaden runtime scope. A later fresh accepted-network setup measurement must prove the chooser before selection.
