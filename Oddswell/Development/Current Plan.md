...
- **Boundary and next gate:** H26Y stops at `recorded_pending_decision`. The next Scope Director review may consider only whether a separate H26L result-link invocation is the smallest truthful next gate; no automatic H26L–H26P chain, settlement, service, backend, or client process path is authorized.
