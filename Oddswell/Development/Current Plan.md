...
- **Boundary and next gate:** H26X stops at `sealed_pending_result`. The next Scope Director review may consider only whether a separate explicit external H26K result invocation is the smallest truthful next gate; no automatic H26K–H26P chain, publication, settlement, service, backend, or client process path is authorized.
