...
- **Implementation:** the existing league exporter publishes the immutable `basketball-match-winner-odds-v1` offer beside unchanged league evidence. At the labeled Sportsbook frontage only, `E` shows both teams, probabilities `57586693`/`42413307`, display odds `1.7365x`/`2.3577x`, `10`–`100` Odds Bucks in increments of `10`, game-start lock, exact gross-return examples, full offer/source identities, and an explicit read-only warning. Leaving the frontage closes the view.
...
- **Next candidate gate:** Scope Director review of the smallest remaining Match Winner requirement. Player wager submission, player-facing history, and correction remain separate unproved gates; Phase 1H.17 has not begun.
